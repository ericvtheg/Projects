...
//Decode Stall
void DecodeStall(struct stage_reg_x *new_x_reg){
...
extern void stage_decode (struct stage_reg_x *new_x_reg)
{
    ...
    //if we mispredict flush, if we are still reading value in Fetch then nop
    //is nop correct? By avoiding doing any operation in X no changes in our
    //soon to be changed register will happen
    if(current_stage_m_register->misprediction == 1 ){
        ...
        DecodeStall(new_x_reg);
    } else { //this works for normal ops but also for cache stalls because decode passes exact same args
            //that was in decode previously...meaning execute is gonna do the same thing twice
        new_x_reg->new_pc       = current_stage_d_register->new_pc;
        new_x_reg->instruction  = current_stage_d_register->instruction;
        new_x_reg->opcode       = current_stage_d_register->opcode;// vs opcode
